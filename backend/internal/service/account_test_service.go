...
// testSerperAccountConnection performs a lightweight real /search probe.
func (s *AccountTestService) testSerperAccountConnection(c *gin.Context, account *Account) error {
	...
	if err != nil {
		return s.sendErrorAndEnd(c, fmt.Sprintf("Invalid Serper base URL: %s", err))
	}
	parsed, err := url.Parse(normalized)
	if err != nil {
		return s.sendErrorAndEnd(c, fmt.Sprintf("Failed to parse Serper base URL: %s", err))
	}
	...
	c.Writer.Flush()
	reqCtx, cancel := context.WithTimeout(WithHTTPUpstreamProfile(ctx, HTTPUpstreamProfileSerper), 30*time.Second)
	defer cancel()
	req, err := http.NewRequestWithContext(reqCtx, http.MethodPost, parsed.String(), strings.NewReader(`{"q":"hello"}`))
	if err != nil {
	...
	if err != nil {
		return s.sendErrorAndEnd(c, fmt.Sprintf("Serper request failed: %s", err))
	}
