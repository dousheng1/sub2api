...
const (
	...
	// Serper returns short JSON responses and does not need HTTP/2 multiplexing.
...
func (s *httpUpstreamService) applyProfilePoolSettings(settings poolSettings, profile service.HTTPUpstreamProfile) poolSettings {
	...
	case service.HTTPUpstreamProfileSerper:
		settings.responseHeaderTimeout = 60 * time.Second
	}
	return settings
}
...
func buildUpstreamTransport(settings poolSettings, proxyURL *url.URL, protocolMode string) (*http.Transport, error) {
	...
	case upstreamProtocolModeSerperH1:
		// Serper edge nodes can return HTTP/2 stream errors; use HTTP/1.1 only.
		transport.ForceAttemptHTTP2 = false
		transport.TLSNextProto = make(map[string]func(string, *tls.Conn) http.RoundTripper)
	}
